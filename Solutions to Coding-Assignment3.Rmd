...
AvgSleepGE8 <- SleepStudy$AverageSleep >= 8
...
AvgSleepGE8_int <- 1*AvgSleepGE8
# Also works: AvgSleepGE8_int <- as.numeric(AvgSleepGE8)
...
SleepStudy$SufficientSleep <- AvgSleepGE8_int 
...
SleepStudy[1:10,c("AverageSleep","SufficientSleep")]
```

**(4)** We can do the same for GPA. In the following code chunk, create a new variable called `LowGPA`. It should be equal to 1 when a student has a GPA that is less than 3 and 0 otherwise. Follow the same coding steps as above for (3), making an needed adjustments.

```{r createLowGPA}
GPA_lt3 <- SleepStudy$GPA < 3
GPA_lt3_int <- 1*GPA_lt3
SleepStudy$LowGPA <- GPA_lt3_int
SleepStudy[1:10,c("GPA","LowGPA")]
```
...
**(6)** What are two additional research questions you could ask to explore the relationship between sleep and academic performance using the variables in the data (including those we created). No coding required here. [2 POINTS]

1. Do students who take early classes go sleep earlier than students who do not take early classes? 

2. Is the relationship between sufficient sleep and GPA different for men than for women? 
...
```{r createSufficientSleep}
SleepStudy$SufficientSleep <- 1*(SleepStudy$AverageSleep >=8)
```
...
```{r ttestStressScore}
ttestEC <- t.test(StressScore ~ LowGPA, data = SleepStudy)
print(ttestEC)
```
