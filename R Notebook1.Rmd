...
```{r dataframes}
# Here, first I am loading an R "package" called "datasets." R packages are the building blocks on top of "base R." They are contributed by wide-ranging organizations and individuals. The datasets package includes many "built-in" datasets that you can use to test out and explore R. When you load the datasets package with "libary(datasets)" all of the datasets in the package become available. 
library(datasets)
...
There are many more data structures in R. You will likely be introduced to more as the class progresses. 